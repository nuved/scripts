...
full_backup() {
        ...
        xtrabackup --backup --history --compress --parallel=1 --compress-threads=1 --target-dir=$BACKUP_DIR/FULL
...
incremental_backup()
{
        ...
        if [ $NUMBER -eq 1 ]
        then
                xtrabackup --backup --history  --incremental --parallel=1 --compress-threads=1 --target-dir=$BACKUP_DIR/inc$NUMBER --incremental-basedir=$BACKUP_DIR/FULL 
        else
                xtrabackup --backup --history --incremental --parallel=1 --compress-threads=1 --target-dir=$BACKUP_DIR/inc$NUMBER --incremental-basedir=$BACKUP_DIR/inc$(($NUMBER - 1)) 
        fi
...
restore()
{
            ...
            xtrabackup --decompress --remove-original --parallel=1 --target-dir=$BACKUP_DIR/FULL 
...
## Parameters
SECRET='password'
BACKUP_DIR=/mnt/mysqldump/xtrabackup/$(date +\%Y-\%m-\%d)
